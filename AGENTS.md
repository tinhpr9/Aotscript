...
4. **Production Release Artifacts**: Immutable GitHub Release assets (`worker-v<version>`).
   - Every asset requires SHA-256 digest validation and signed GitHub Artifact Attestation (SLSA provenance) generated by `actions/attest-build-provenance` pinned to an exact immutable commit SHA.
   - Attestations are locally verified via `gh attestation verify` before release publication is permitted.
