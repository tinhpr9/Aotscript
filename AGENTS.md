...
## Automated Dependency Management and Renovate Policy (Phase 4)

- **Single Update Authority**: Renovate is the single dependency version-update authority for this repository. Overlapping automated dependency update bots (such as Dependabot version updates) must not run simultaneously.
- **Strict No-Automerge Contract**: Renovate NEVER automerges (`automerge: false`, `platformAutomerge: false`). All dependency updates arrive as pull requests and require explicit maintainer authorization and CI verification. Direct writes to `main` are prohibited.
- **Full 40-hex commit SHA pinning**: All external GitHub Actions `uses:` references must use full 40-hex commit SHAs accompanied by verified human-readable inline version comments (`# vX.Y.Z`). Renovate maintains action digest updates without reverting to mutable tags (`@main`, `@v1`, etc.).
- **Lockfile Ownership**: `package-lock.json` files are strictly generated and synchronized by the package manager (`npm ci` / `npm install`); manual edits to lockfile dependency graphs are prohibited.
- **Risk-Gated PR Isolation**: High-risk production deployment and supply-chain dependencies remain isolated into dedicated, single-dependency pull requests (`wrangler`, `cloudflare/wrangler-action`, `actions/attest-build-provenance`).
- **Promptfoo Custom Regex Manager**: Promptfoo CLI versions in `.github/workflows/promptfoo-benchmark.yml` are managed via a scoped regex custom manager (`npm install -g promptfoo@<version>`).
- **Runtime Version Gating**: Major and runtime-family version changes (Node.js, Python) require Dependency Dashboard approval.
- **Worker Release Independence**: Dependency update PRs must NOT modify worker release metadata (`worker-release.json`, manifests, release SHA-256) unless explicitly executing an intentional release-preparation workflow.
...
- **Phase 4 (In Progress — configuration)**: Automated dependency management (Renovate).
