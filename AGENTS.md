...
3. **Resumable Immutable Drafts**:
   A populated immutable draft may resume if and only if release identity, `TARGET_COMMIT`, complete 14-asset set, sizes, and SHA-256 digests are independently revalidated against deterministic reproduction. Never delete or overwrite immutable release assets to recover from transient attestation or network failures.
4. **Final Pre-Publish Recheck Gate**:
   Immediately before `draft=false`, re-fetch the exact draft JSON, revalidate release identity and complete asset set/digests, revalidate attestation subjects against exact `TARGET_COMMIT` source digest, and publish immediately (TOCTOU gap minimized to 0).
5. **Deployment Authority and PR Isolation**:
   PR and feature branches must NEVER deploy to production AOT Hub. Exactly ONE canonical production deployment authority is permitted (`.github/workflows/deploy-hub.yml` on `main`). Cloudflare Workers Builds / Git integration performs PR build validation only and must not mutate live production services.
