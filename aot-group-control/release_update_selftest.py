...
with tempfile.TemporaryDirectory(prefix="aot-release-download-") as folder:
    ...
    module._download("https://github.com/tinhpr9/Aotscript/releases/download/worker-v2026.08.14.01/asset", target, len(content))
    ...
    for error in (
        ...
        try:
            module._download("https://github.com/tinhpr9/Aotscript/releases/download/worker-v2026.08.14.01/asset", target)
    ...
    for name in sorted(module.REQUIRED_FILES):
        ...
        release_files.append({
            "path": name,
            "url": f"https://github.com/tinhpr9/Aotscript/releases/download/worker-v2026.08.14.01/{name}",
            "size": len(body), "sha256": hashlib.sha256(body).hexdigest(),
            "github_digest": "sha256:" + hashlib.sha256(body).hexdigest(),
        })
    manifest = {
        "schema_version": 3, "worker_version": "aot-worker-2026.08.14.01",
        "tag": "worker-v2026.08.14.01", "commit_sha": "a" * 40,
    ...
    metadata = {
        "version": "aot-worker-2026.08.14.01", "tag": "worker-v2026.08.14.01",
        "commit_sha": "a" * 40,
        "manifest": {
            "name": "worker-manifest.json",
            "url": "https://github.com/tinhpr9/Aotscript/releases/download/worker-v2026.08.14.01/worker-manifest.json",
    ...
    assert loaded["version"] == "aot-worker-2026.08.14.01" and loaded["channel"] == "canary"
