...
controller_src = controller_src.replace('BACKUP_STARTED', 'RESTORE_STARTED')

func_start = controller_src.find("def backup_restore_data(")
helpers_end = controller_src.find("def open_swift_apps(")
if func_start != -1 and helpers_end != -1:
...
def _tap_wait(bounds: Bounds, deadline: float | None):
    before_fp = hashlib.md5(pathlib.Path("/sdcard/window.xml").read_bytes()).hexdigest() if pathlib.Path("/sdcard/window.xml").exists() else ""
    ...
    while time.time() < end:
        ...
        time.sleep(0.5)
        dump_ui_xml()
        after_fp = hashlib.md5(pathlib.Path("/sdcard/window.xml").read_bytes()).hexdigest() if pathlib.Path("/sdcard/window.xml").exists() else ""
...
"""
    controller_src = controller_src[:func_start] + new_logic + controller_src[helpers_end:]
    with open(controller_path, "w") as f:
        f.write(controller_src)

for path in ["aot-group-control/relay.py", "cloudflare-worker/fleet-state.js", "cloudflare-worker/worker.js", "aot-group-control/relay_selftest.py", "tests/test_backup_restore_data.py"]:
    full_path = ROOT / path
